...
for folder in */; do
    ...
    if [[ -f "${folder}examprep/index.md" ]]; then
        # Read the content of index.md
        content=$(cat "${folder}examprep/index.md")
        ...
        subject_name=$(sed -n 's/^# //p' "${folder}examprep/index.md" | head -n 1 | sed 's/[[:space:]]*$//')  # Remove leading # and trailing whitespace
        echo "Processing ${subject_name}"

        # Generate the new entry
        new_entry="- [${subject_name}](${folder}examprep/index.md)"
